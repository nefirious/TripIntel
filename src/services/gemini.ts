    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isRetryable && attempt < maxRetries) {
          ...
          console.warn(`Gemini API busy with ${model} (attempt ${attempt + 1}/${maxRetries + 1}). Retrying in ${Math.round(delay)}ms...`);
          await sleep(delay);
          continue;
        }
        
        // If it's not retryable or we've exhausted retries for this model, try the next model
        console.warn(`Model ${model} failed for snapshot, trying next model or failing.`);
        break;
      }
    }
  }

  throw lastError || new Error("The travel advisor is currently overwhelmed. Please try again in a few moments.");
}
